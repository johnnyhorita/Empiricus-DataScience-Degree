...
tp_calculo = st.sidebar.selectbox('Calculadora', ("Titulos NTN-F", "Titulos LTN"))
...
if tp_calculo == "Titulos NTN-F": 
    tp_calc_desc = "Nota do Tesouro Nacional Série F (NTN-F)"
elif tp_calculo == "Titulos LTN": 
    tp_calc_desc = "Letra do Tesouro Nacional (LTN)"
    
st.subheader(tp_calc_desc)

if run_button:
    if dt_vencimento <= dt_entrada:
        data_load_state = st.error('A data de vencimento é menor ou igual a data de entrada. Preencha as datas corretamente.')
            
    else:
        data_load_state = st.info('Preparando informações...')
        load_state = st.text('')
        ...
        data_load_state.info('Calculando informações...')
        load_state.text('')
        ...
        if tp_calculo == "Titulos NTN-F":
            ...
            my_bar.progress(percent_complete)
            data_load_state.success('Calculo efetuado!')
            
        elif tp_calculo == "Titulos LTN":
            vl_nominal = 1000
            D1=1
            df_fluxo, vl_principal, vl_pu_ntnf = ntnF.Calcula_PU_LTN(dt_entrada_str, dt_vencimento_str, vl_nominal, tx_anual, D1)
            percent_complete += (0.7)
            my_bar.progress(percent_complete)

            df_fluxo.rename(columns = {'Data_Entrada': 'Data Entrada', 'Data_Compra': 'Data Compra', 'Data_Pagamento': 'Data Vencimento'}, inplace = True)
            st.subheader(f'Tesouro Prefixado {dt_vencimento.strftime("%Y")}')
            st.write(f'Valor do Preço Unitário: {vl_pu_ntnf}')
            st.dataframe(df_fluxo)
            percent_complete += (0.1)
            my_bar.progress(percent_complete)
